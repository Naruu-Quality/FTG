...
if(((nearestObject [[23751.881,16250,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[23752.334,16249.799,0],"Land_Dome_Small_F"]) == _building) && ({side _x == west} count playableUnits < (LIFE_SETTINGS(getNumber,"cops_online_min")))) exitWith {
	hint format [localize "STR_Civ_NotEnoughCops",(LIFE_SETTINGS(getNumber,"cops_online_min"))]
};
if((typeOf _building) == "Land_Dome_Small_F" && life_fed_break == 0) exitWith {hint localize "STR_ISTR_Bolt_Exploit"};
if(isNil "life_boltcutter_uses") then {life_boltcutter_uses = 0;};

if((nearestObject [[23751.881,16250,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[23752.334,16249.799,0],"Land_Dome_Small_F"]) == _building) then {
...
switch (typeOf _building) do {
	...
	case "Land_Dome_Small_F": {life_fed_break = 2;};
};
